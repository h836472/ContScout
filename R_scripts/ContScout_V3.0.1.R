...
#change of input switches
#addition of Diamond
#resume previous analysis like OrthoFinder
#proteins that are not present in the gff shall be labelled and excluded as contamination
#singularity exec -B /work/balintb/databases:/databases -B /work/tmp/:/cs_temp -B /work/balintb/ContScout_Devilopment/resume_run:/data docker://h836472/contscout_avx2:latest ContScout -a mmseqs -u /databases -c 24 -d uniref100 -i Acanthamoeba_castellanii_str._Neff_tax_1257118_04Aug_2022_03_47/ -q 1257118 -r -x all -p 20 -t /cs_temp
...
option_list = list(
  ...
  make_option(c("-m", "--splitmemlimit"), type="character", default=NULL, 
              help="Memory split limit for MMSeqs", metavar="splitmemlimit"),
...
opt[["userdir"]]="/work/balintb/databases" #-u 
opt[["cpu"]]=32 #-c 
opt[["dbname"]]="uniref100" #-d 
opt[["inputdir"]]="/work/balintb/ContScout_EveryThing/NO_GFF/Acidobacterium_capsulatum_ATCC_51196_tax_240015_02Aug_2022_15_17" # -i
opt[["querytax"]]=2788 #-q 
opt[["force"]]=FALSE
opt[["reuse_abc"]]=TRUE #-r
opt[["no_annot"]]=TRUE
opt[["sens"]]=2 #-s 2
opt[["splitmemlimit"]]="150G" #-m 200G
opt[["exclude_tax"]]="all" #-x all
opt[["list_databases"]]=FALSE
opt[["pci"]]=20 #-p 20
opt[["tmpdir"]]="/SSD1/tmp" #-t /SSD1/tmp
...
{
byedie(paste0('Please specify an input directory that contains the protein sequence and annotation file!'))
}
startdir=getwd()
setwd(opt[["inputdir"]])
time_start=Sys.time()
...
db.info=db.info[,c("db.Name","db.CRC","db.Loc","date","is.Latest")]
...
qdb=gsub(".+/","",qdb)
MM.qdb.cmd=paste0("mmseqs createdb ",protfile," ",outdir,"/",qdb)
cat("Converting query sequences to MMSeqs database format\n")
system(MM.qdb.cmd)

##perform search
MM.search.cmd=paste0("mmseqs search -s ",opt[["sens"]]," -a --threads ",opt[["cpu"]]," ",outdir,"/",qdb," ",db.loc.sel," ",outdir,"/TaxMMSeqs.resDB"," ",opt[["tmpdir"]])
if(!is.null(opt[["splitmemlimit"]]))
{
MM.search.cmd=paste0(MM.search.cmd," --split-memory-limit ",opt[["splitmemlimit"]])
}
...
Trafo.cmd=paste0("mmseqs convertalis ",outdir,"/",qdb," ",db.loc.sel," ",outdir,"/TaxMMSeqs.resDB"," ",outdir,"/TaxMMSeqs.ABC --format-output query,target,bits,qlen,nident --threads ",opt[["cpu"]])
system(Trafo.cmd)
cat("Now reading Alignment result (ABC) file.\n")
